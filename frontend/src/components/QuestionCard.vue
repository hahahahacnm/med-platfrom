...
const parseText = (text: string) => {
    if (!text) return ''
    // 替换 Markdown 图片语法 ![alt](url) -> <img src="...">
  ...
  <div :id="`question-anchor-${question.id}`" class="scroll-anchor">
    <n-card class="q-card" hoverable content-style="padding-bottom: 0;">
      ...
      <div v-if="hasChild">
          ...
          <div v-if="question.type.includes('B1') && opts" class="b1-ctx">
             <div class="b1-h">（共用备选答案）</div>
             <div class="b1-list">
                 <div v-for="(t, k) in opts" :key="k" class="b1-row">
