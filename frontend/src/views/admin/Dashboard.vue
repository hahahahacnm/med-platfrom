...
const formatStatus = (status: string): { type: 'warning' | 'success' | 'error' | 'default', text: string } => {
  switch(status) {
...
<template>
  <div class="dashboard-container">
        ...
        <n-table :bordered="false" :single-line="false">
          ...
          <tbody>
            <tr v-for="item in stats.recent_sales" :key="item.ID">
              <td style="font-family: monospace;">{{ (item.OrderNo || item.order_no || '').substring(0,8) }}...</td>
              <td>{{ item.description || '赞助分润' }}</td>
              <td>¥{{ (item.OrderAmount || item.order_amount || 0).toFixed(2) }}</td>
              
              <td style="color: #d03050; font-weight: bold;">
                +¥{{ (item.Profit || item.profit || 0).toFixed(2) }}
                <span style="font-size: 12px; color: #999; margin-left: 4px;">
                  ({{ ((item.Profit || item.profit || 0) / (item.OrderAmount || item.order_amount || 1) * 100).toFixed(0) }}%)
                </span>
              </td>
    ...
    <div v-else-if="userStore.role === 'admin'">
      ...
      <n-card title="🛍️ 赞助动态 (最新10笔)" style="margin-top: 20px;">
        <n-table size="small">
          <thead>
            <tr>
              <th>时间</th>
          ...
          <tbody>
            <tr v-for="order in stats.recent_orders" :key="order.order_no">
              ...
              <td>{{ order.username || 'User-' + (order.UserID || order.user_id) }}</td>
              <td><n-tag size="tiny" type="info">{{ order.product_name || '赞助充值' }}</n-tag></td>
              <td style="font-weight: bold; color: #18a058;">+¥{{ (order.Amount || order.amount || 0).toFixed(2) }}</td>
              <td style="font-family: monospace;">{{ (order.OrderNo || order.order_no || '').substring(0,8) }}...</td>
              <td>
                <n-tag size="small" :type="(order.Status || order.status) === 'PAID' ? 'success' : 'warning'">
                    {{ order.Status || order.status }}
                </n-tag>
              </td>
            </tr>
            <tr v-if="!stats.recent_orders || stats.recent_orders.length === 0">
              <td colspan="6" style="text-align: center; color: #999;">暂无赞助记录</td>
            </tr>
          </tbody>
        </n-table>
      </n-card>
        ...
        <n-table>
          ...
          <tbody>
            <tr v-for="item in stats.withdraw_list" :key="item.ID">
              ...
              <td style="font-weight: bold; font-size: 16px;">¥{{ (item.amount || 0).toFixed(2) }}</td>
              <td>
                <n-popover trigger="hover">
                  <template #trigger>
                    <n-icon size="24" style="cursor: pointer; color: #2080f0"><QrCodeOutline /></n-icon>
                  </template>
                  <img :src="getImageUrl(item.payment_image)" style="width: 200px; height: 200px; object-fit: contain;">
                </n-popover>
              </td>
    ...
    <n-modal v-model:show="showWithdrawModal" preset="card" title="申请提现" style="width: 400px">
      ...
      <div v-if="!isEditingPayment && withdrawForm.payment_image" style="text-align: center;">
        ...
        <n-image 
          width="200" 
          :src="getImageUrl(withdrawForm.payment_image)" 
